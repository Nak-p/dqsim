...
//
// 移動速度は game_config.json の character_travel_speed を参照する。
...
namespace AgentSim.Pathfinding
{
    public static class TravelCalculator
    {
        ...
        /// 移動速度は SettingsRegistry.Current.Game.character_travel_speed を自動参照する。
        /// </summary>
        public static float CalculateTravelHours(List<Vector2Int> path, MapData map)
        {
            float speed = SettingsRegistry.Current?.Game?.character_travel_speed ?? 1f;
